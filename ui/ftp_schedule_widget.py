...
# ⬇️ WICHTIG: Die Klasse heißt TransferPlanWidget (nicht FtpPlanWidget)
from ui.ftp_plan_widget import TransferPlanWidget
from ui.ftp_plan_dialog import TransferPlanDialog
from ui.transfer_queue_panel import TransferQueuePanel   # eingebettete Queue
...
class FtpScheduleWidget(QtWidgets.QWidget):
    ...
    def add_plan(self):
        ...
        if dlg.exec() == QtWidgets.QDialog.Accepted:
            p = dlg.plan_data
            p.setdefault("last_run", "")
            p.setdefault("completed_once_at", "")
    ...
    def edit_plan(self, plan: dict):
        ...
        if dlg.exec() == QtWidgets.QDialog.Accepted:
            ...
            self._normalize_schedule(upd)
            update_transfer_plan(upd)
            self.load_plans()
    ...
    def _start_in_panel(self, plan: dict, bring_to_front: bool):
        if self.queue_panel.is_busy():
            if QtWidgets.QMessageBox.question(
                self,
                "Laufender Transfer",
                "Ein Transfer läuft bereits. Trotzdem neuen starten?",
                QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
            ) != QtWidgets.QMessageBox.Yes:
                return
    ...
    def _is_due(self, plan: Dict, now: datetime) -> bool:
        ...
        if stype == "daily":
            ...
            if not self._in_window(tgt, now, self._tolerance):
                return False
        ...
        if stype == "weekly":
            if now.isoweekday() != sched_dt.isoweekday():
                return False
            tgt = now.replace(hour=h, minute=m, second=0, microsecond=0)
            if not self._in_window(tgt, now, self._tolerance):
                return False
