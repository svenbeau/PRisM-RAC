...
class TransferPlanDialog(QtWidgets.QDialog):
    ...
    Abwärtskompatible Signatur:
        __init__(plan_data, manager=None, parent=None)

    Unterstützte/erwartete Felder (wie in deinen Logs/JSON):
      - name
      - source_is_ftp (bool)
      - source_ftp_server (str), source_remote_path (str), source_path (str), source_remote_archive (str)
      - use_ftp (bool), ftp_server (str), target_path (str)
      - versioning_mode ("mirror"|"suffix"), suffix_format (str)
      - retry_count (int), verify_mode ("size_only"|"md5")
      - schedule_type ("once"|"daily"|"weekly"), schedule_time ("yyyy-MM-dd HH:mm")
      - move_after (str)
      - auto_delete_after_move_enabled (bool), auto_delete_after_move_hours (int)
    """

    def __init__(self, plan_data, manager=None, parent=None):
        super().__init__(parent)
        self.plan_data = plan_data if isinstance(plan_data, dict) else {}
        self.manager = manager  # optional, wird hier nicht benötigt
    ...
    def init_ui(self):
        ...
        self.src_is_ftp_check.stateChanged.connect(self.on_src_ftp_toggled)
        ...
        self.ftp_check.stateChanged.connect(self.on_dst_ftp_toggled)
    ...
    def load_plan_data(self):
        ...
        # Falls verify_mode nicht gesetzt ist → size_only
        self.verify_combo.setCurrentText(pd.get("verify_mode", "size_only"))
        ...
        self.on_src_ftp_toggled()
        self.on_dst_ftp_toggled()
