...
from ui.json_explorer import JSONEditorDialog  # Wir verwenden den EditorDialog aus dem bisherigen json_explorer.py
...
class JSONExplorerWidget(QtWidgets.QWidget):
    ...
    Ein Widget, das:
      ...
      - Zusätzlich einen 'Browse'-Button, um per QFileDialog einen neuen Ordner auszuwählen
    ...
    def __init__(self, settings, parent=None):
        ...
        # Wir nehmen an, dass die "recent_dirs" in den Settings stehen
    ...
    def init_ui(self):
        ...
        # Oberer Bereich: Ordner-Auswahl (Dropdown) + Browse-Button
    ...
    def browse_for_folder(self):
        """
        Öffnet einen QFileDialog, um einen neuen Ordner auszuwählen.
        Falls gewählt, wird er in die recent_dirs eingetragen (an erster Stelle),
        und die Liste der JSON-Dateien wird neu geladen.
        """
        folder = QtWidgets.QFileDialog.getExistingDirectory(self, "Ordner auswählen", self.folder_combo.currentText())
        if folder:
            # Falls der Ordner noch nicht in recent_dirs ist, füge ihn hinzu
            if folder not in self.recent_dirs:
                self.recent_dirs.insert(0, folder)
                self.folder_combo.insertItem(0, folder)
                self.folder_combo.setCurrentIndex(0)
            else:
                # Falls bereits vorhanden, wähle ihn einfach aus
                idx = self.folder_combo.findText(folder)
                self.folder_combo.setCurrentIndex(idx)

            # Optional: In settings übernehmen und speichern
    ...
    def on_back(self):
        """
        Falls Du möchtest, dass "Zurück" wieder zum Hotfolder wechselt,
        kannst Du hier z. B. den QStackedWidget-Index setzen.
        """
