...
// fm
/*
@@@BUILDINFO@@@ SuperSkript.jsx 1.0.0.0
*/
...
<javascriptresource>
<name>rcm-Super-Skript</name>
<menu>automate</menu>
<category>rcm_RefScan</category>
<enableinfo>true</enableinfo>
<eventid>3caa3434-cb67-11d1-bc43-0060b0a13ab2</eventid>
<terminology><![CDATA[<< /Version 1
                         /Events <<
                          /3caa3434-cb67-11d1-bc43-0060b0a13ab2 [(rcm-Super-Skript) /imageReference <<
...
function loadConfiguration() {
    ...
    // Konfigurationsdatei ist in /config/script_config.json
    var configPath = new File(projectRoot + "/config/script_config.json");
    
    if (DEBUG_OUTPUT) {
        alert("Looking for config at: " + configPath.fsName);
    }
    ...
    // JSON-Parser ist in importFunctions() enthalten
    importFunctions();
    ...
    try {
        ...
        var scripts = (fullConfig.scripts && fullConfig.scripts.constructor === Array) ? fullConfig.scripts : [];
        var currentScriptPath = scriptFile.fsName;
        
        if (DEBUG_OUTPUT) {
            alert("Current script path: " + currentScriptPath);
            alert("Found " + scripts.length + " script configurations");
        }
        
        // Suche nach der Konfiguration für dieses Skript
        for (var i = 0; i < scripts.length; i++) {
            // Normalisiere Pfade für Vergleich (Handling von Pfadtrenner-Unterschieden)
            var configScriptPath = scripts[i].script_path.replace(/\\/g, "/");
            var thisScriptPath = currentScriptPath.replace(/\\/g, "/");
            
            if (DEBUG_OUTPUT) {
                alert("Checking: " + configScriptPath);
            }
            
            // Vergleiche den Dateinamen als Fallback
            var configScriptName = configScriptPath.split("/").pop();
            var thisScriptName = thisScriptPath.split("/").pop();
            
            if (thisScriptPath.indexOf(configScriptPath) !== -1 || 
                configScriptPath.indexOf(thisScriptPath) !== -1 || 
                configScriptName === thisScriptName) {
                
                if (DEBUG_OUTPUT) {
                    alert("Found configuration for: " + scripts[i].script_path);
                }
                return scripts[i];
            }
        }
        
        // Keine passende Konfiguration gefunden
        alert("No configuration found for this script in script_config.json");
        return {};
    } catch (e) {
...
// Prüfen, ob das Skript bereits ausgeführt wurde
if (scriptHasRun) {
    if (DEBUG_OUTPUT) {
        alert("Script has already run - preventing duplicate execution");
    }
} else {
    scriptHasRun = true;
...
function ResizeTheImage(width, height) {
    ...
    try {
        ...
        if (!recipesPath) {
            // Verwende den Standard-Pfad, falls keiner in der Konfiguration angegeben ist
            try {
                var configjsonpath = (new File($.fileName)).parent + '/config/rcmConfig.json';
                ...
                recipesPath = localConfig.recipesPath;
            } catch (e) {
                alert("No recipes path found in configuration: " + e);
                return true;
            }
        }
        
        if (DEBUG_OUTPUT) {
            alert("Using recipes path: " + recipesPath);
        }
        ...
        // Read the source and target from the file
        ...
        // check all Recepis in recipeFolder for their tags and compare to the one needed
        ...
        if (targetRecipe == null) {
            throw new Error("the right combination of target and source have not been found: DocSource: " + docSource + " DocTarget: " + docTarget);
            return true;
        }
        
        // Verwende den Action-Ordnernamen aus der Konfiguration
        var actionFolderName = config.actionFolderName;
        
        if (!actionFolderName) {
            actionFolderName = "Grisebach 2025"; // Fallback-Wert
        }
        
        if (DEBUG_OUTPUT) {
            alert("Using action folder name: " + actionFolderName);
        }
        
        /*
        * This is for each output in the json of target Recipe ( )
        */
        for (var i = 0; i < targetRecipe.outputs.length; i++) {
            ...
            //alert("action: "+currentOutput.photoshopAction +" from: "+ config.actionFolderName)
            ...
            if (DEBUG_OUTPUT)
                alert("name: " + currentOutput.name + "\n" + "action: " + action + " from: " + actionFolderName + "\n\nsave as:" + newName + " in: " + outputFolder);
            
            // Erstelle den Ausgabeordner, falls er nicht existiert
            var folderObject = new Folder(outputFolder);
            if (!folderObject.exists) {
                folderObject.create();
                if (DEBUG_OUTPUT)
                    alert("Created output folder: " + outputFolder);
            }
            
            if (currentOutput.filetype == "jpg") {
                saveJpeg(documentCopy, outputFolder, newName);
            }
            else if (currentOutput.filetype == "tif") {
                saveTiff(documentCopy, outputFolder, newName);
            }
            else if (currentOutput.filetype == "png") {
                savePng(documentCopy, outputFolder, newName);
            }
            else if (currentOutput.filetype == "psd") {
                SavePSD(documentCopy, outputFolder, newName);
            }
...
function saveJpeg(document, path, filename){
    if(DEBUG_OUTPUT)
        alert("save a JPEG");
    ...
    document.saveAs(jpgFile, jpgSaveOptions, true, Extension.LOWERCASE);
}

function saveTiff(document, path, filename){
    if(DEBUG_OUTPUT)
        alert("save a TIFF");
    ...
    var tifFile = new File(outputFolder + "/" + filename);
    ...
    document.saveAs(tifFile, tiffSaveOptions, true, Extension.LOWERCASE);
}

function savePng(document, path, filename){
    if(DEBUG_OUTPUT)
        alert("save a PNG");
    ...
    document.saveAs(pngFile, pngSaveOptions, true, Extension.LOWERCASE);
}

function SavePSD(document, path, filename)
{
    if(DEBUG_OUTPUT)
        alert("Save a PSD");
    ...
    var psdFile = new File(outputFolder + "/" + filename);
...
function SaveOffParameters(sizeInfo) {
    try {
        ...
        app.putCustomOptions("3caa3434-cb67-11d1-bc43-0060b0a13ab2", d);
...
function GlobalVariables() {
    ...
    strTitle = "Grisebach Super Skript";
...
function FitImage() {
    ...
    this.InitVariables = function() {
        ...
        try {
            var desc = app.getCustomOptions("3caa3434-cb67-11d1-bc43-0060b0a13ab2");
        ...
        if(fromAction){
            ...
            var tempSizeInfo = new SizeInfo();
            // set the playback options to sizeInfo
            descriptorToObject(tempSizeInfo, app.playbackParameters, strMessage);
            sizeInfo = tempSizeInfo;
        }
